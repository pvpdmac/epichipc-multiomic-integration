...
my_pal <- c("#4DBBD5FF", "#E64B35FF", "#3C5488FF", "steelblue")
...
# query
query <- readr::read_tsv('results/tables/sigora_pathways.tsv')
reactome_df <- msigdbr::msigdbr(species = "Homo sapiens", category = "C2", subcategory = "CP:REACTOME")
query <- reactome_df %>% 
  filter(gs_exact_source %in% query$pathway_id) %>% 
  distinct(gs_name, gs_exact_source, gs_description) %>% 
  pull(gs_name)

msigdb <- sear::collections %>%
  filter(collection == 'C2:CP:REACTOME', geneset %in% query) %>%
...
# fix to label with GEO IDs 2026/03/09
con <- con %>% mutate(pub = ifelse(grepl('^PMID', pub), geo, pub))
...
make_ecdf   <- function(df, col = NULL, unique_paths = NULL) {
  ...
  my_cols <- c("#CCCCCC", "#808080" , "#333333", col)
...
make_cor    <- function(df, col = NULL, unique_paths = NULL) {
  ...
  ht <- ht %>% 
    ComplexHeatmap::Heatmap(
      ...
      col = circlize::colorRamp2(breaks = c(-0.7, 0, 1), colors = c('brown4', 'white', 'cornflowerblue'))
    )
...
make_upset  <- function(df, col = NULL, unique_paths = NULL) {
  ...
  tmp <- df %>%
    ...
    unnest(gsea) %>%
    filter(padj < 0.1) %>%
    transmute(pub, pathway, score = 1)
  
  i  <- unique(tmp$pub)
  tmp <- spread(tmp, pub, score, fill = '0') %>% mutate(across(all_of(i), as.numeric))
  
  ComplexUpset::upset(data = tmp, intersect = i)
    ...
    ComplexUpset::upset(
      ...
      set_sizes = (
        ComplexUpset::upset_set_size(position = 'right') +
          ...
          geom_text(aes(label=after_stat(count)), hjust = -0.1, size = 4, stat = 'count') +
      ...
      queries = list(
        ComplexUpset::upset_query(
          fill  = col,
          color = col,
          intersect = c('GSE65834', 'GSE155198', df %>% filter(grepl('MA', pub)) %>% pull(pub))
        )
      ),
      
      base_annotations = list(
        ' ' = ComplexUpset::intersection_size(
          text = list(size = 4, check_overlap = F),
          counts = T
        ) +
...
make_bubble <- function(df, col = NULL, unique_paths = NULL) {
  ...
  # select top 30 most significant across all datasets using pearson's method
  ...
  keep <- tmp %>% pull(pathway) %>% unique() %>% head(n = 35)

  if(!is.null(unique_paths)) {
    # instead, force pathways uniquely identified in neonates in
    tmp <- tmp %>% arrange(desc(pathway %in% unique_paths), desc(range)) %>% filter(pathway %in% keep)
  } else {
    tmp <- tmp %>% filter(pathway %in% keep)
  }

  # fix long names of REACTOME
  tmp <- tmp %>%
    mutate(
      pathway = pathway %>%
        gsub('REACTOME_', '', .) %>%
  ...
  # bold modules
  unique_paths <- c('NEUTROPHIL DEGRANULATION', 'INTERLEUKIN 1 SIGNALING', 'INTERLEUKIN 4 AND INTERLEUKIN 13 SIGN...', 'PLATELET ACTIVATION SIGNALING AND AGG...', 'mod1 (PMID:26742691)', 'mod16 (PMID:26742691)')
  ...
  tmp %>%
    ...
    theme(
      ...
      # bold modules
      axis.text = element_text(size = 9, face = labels),
    ...
    annotate('segment', x =  0.5, y = 37.5, xend =  2, yend = 37.5, arrow = arrow(type = 'closed', length = unit(0.01, 'npc'))) +

    annotate('text',    x = -0.5, y = Inf, hjust = 1, vjust = 1, label = 'Higher in non-responders', size = 3, fontface = 1) +
    annotate('segment', x = -0.5, y = 37.5, xend = -2, yend = 37.5, arrow = arrow(type = 'closed', length = unit(0.01, 'npc'))) +

    annotate('segment', x = 0, y = 0, xend = 0, yend = 35.5, linewidth = 0.1) +
...
# filter
# con <- con %>% filter(grepl('MA\\+|GSE', pub)) %>% mutate(pub = gsub('MA\\+', '', pub))

# subset and remove other timepoints for GSE155198
dats <- tibble(model = c('MA+DOL128'), col = 'steelblue')

dats$subset <- dats %>%
  pmap(\(model, ...) {
    con %>% 
      filter(grepl('GSE', pub) | pub == !!model)  %>% 
...
figs$unique_paths <- figs %>% pmap(\(comb_mat, ...) ComplexHeatmap::extract_comb(comb_mat, comb_name = '001'))
...
layout <- c(
  ...
  area(t = 1, b = 10,  l = 6, r = 12),
  area(t = 8, b = 20, l = 1, r = 12)
)

bot <- patchwork::wrap_plots(
  patchwork::wrap_plots(
    A = free(figs$facet[[1]] + theme(legend.position = 'none')),
    B = figs$facet[[2]], 
    C = figs$facet[[3]], 
    design = layout
  ), 
...
bot + patchwork::plot_annotation(tag_levels = list(c('a', 'c', 'b', '', '', 'd')))
