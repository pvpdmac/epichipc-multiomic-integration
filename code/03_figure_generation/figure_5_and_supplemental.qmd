...
title: |
        **BTM**  \
        Figure 5, Supplemental Figure S10
format:
  html:
    fig-format: svg
    filters:
...
my_pal <- c("#4DBBD5FF", "#E64B35FF", "#3C5488FF", "#00A087FF")
...
make_bubble <- function(df, col = NULL, unique_paths = NULL) {
  ...
  if(!is.null(unique_paths)) {
    ...
    tmp <- tmp %>% arrange(desc(pathway %in% unique_paths), desc(range)) %>% slice_head(n = 140)
  } else {
    tmp <- tmp %>% filter(row_number()<= 140)
  }
...
top <- patchwork::wrap_plots(
  patchwork::wrap_plots(
    A = free(tmp$facet[[1]] + theme(legend.position = 'none')),
    B = tmp$facet[[2]], 
    C = tmp$facet[[3]], 
    design = layout
  ), 
